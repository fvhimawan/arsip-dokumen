...
def save_metadata():
    ...
    category = request.form.get('custom_category') or request.form.get('category')
    ...
    c = conn.cursor()
    c.execute('''
        INSERT INTO documents (filename, category, doc_type, company_name, issued_date, notes)
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (filename, category, doc_type, company_name, issued_date, notes))
    conn.commit()
    conn.close()
...
def list_documents():
    ...
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("SELECT DISTINCT category FROM documents WHERE category IS NOT NULL")
    ...
    documents = [row for row in rows if os.path.exists(os.path.join(UPLOAD_FOLDER, row[1]))]
...
def edit_document(doc_id):
    ...
    if request.method == 'POST':
        ...
        c.execute('''
            UPDATE documents
            SET category=?, doc_type=?, company_name=?, issued_date=?, notes=?
            WHERE id=?
        ''', (category, doc_type, company_name, issued_date, notes, doc_id))
        conn.commit()
        conn.close()
        return redirect('/documents')
    ...
    if row:
        ...
        if count == 1:
            try:
                ...
                conn.close()
                return f"<h3>❌ File '{filename}' sedang dibuka. Tutup terlebih dahulu dan coba lagi.</h3><a href='/documents'>← Kembali</a>"
